...
export function GateHealth({ status, traceStatus, controlStatus, streamDropped, actionable = false, busy, onPause, onResume, onDrain, onDisarm }: {
  ...
  return (
    <section className="border border-slate-800 bg-slate-900/70 p-3 text-xs text-slate-400">
      ...
      <details className="mt-3 border-t border-slate-800 pt-3"><summary>Capacity, loss &amp; audit diagnostics</summary><div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 font-mono text-[11px]">
      ...
      {status?.armed ? (
        <div className="mt-3 grid grid-cols-2 gap-2 border-t border-slate-800 pt-3">
          {status.paused ? <Action busy={!actionable || busy('resume')} label="Resume intake" onClick={onResume} /> : <Action busy={!actionable || busy('pause')} label="Pause intake" onClick={onPause} />}
          <Action busy={!actionable || busy('drain')} label="Drain held" onClick={onDrain} />
          <button className="col-span-2 border border-rose-600 px-2 py-1.5 font-bold text-rose-300 hover:bg-rose-950 disabled:opacity-40" disabled={!actionable || busy('disarm')} onClick={onDisarm} type="button">{busy('disarm') ? 'Disarming…' : 'Emergency disarm'}</button>
        </div>
      ) : null}
    </section>
  )
...
function Metric({ label, value, danger = false }: { label: string; value?: number; danger?: boolean }) {
  return <span className="flex justify-between gap-2"><span>{label}</span><b className={danger && value ? 'text-rose-300' : 'text-slate-200'}>{value ?? '—'}</b></span>
}
