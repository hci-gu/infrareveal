  ...
  return (
    <section aria-label="Current approval queue" className="border border-slate-800 bg-slate-900/70">
      <header className="flex items-center justify-between border-b border-slate-800 px-3 py-2">
        <div><h2 className="text-sm font-semibold text-slate-100">Approval queue <span className="lab-now">NOW</span></h2><p className="text-[11px] text-slate-500">Live control state, never rewound</p></div>
        <span className="bg-rose-950 px-2 py-1 font-mono text-[10px] font-bold text-rose-300">NOW · {actionable || ordered.length ? ordered.length : '—'}</span>
      </header>
      ...
      {gateMode === 'strict' ? (
        <div className="m-3 flex items-center gap-2 border border-violet-900 bg-violet-950/20 p-2 text-xs text-violet-200">
          ...
          <button className="border border-violet-600 px-2 py-1 disabled:opacity-40" disabled={!actionable || inFlight.size > 0} onClick={() => onAcceptNext(acceptCount)} type="button">Step packets</button>
        </div>
      ) : ordered.length > 1 ? <button className="m-3 border border-emerald-700 px-2 py-1 text-xs text-emerald-300 disabled:opacity-40" disabled={!actionable || inFlight.size > 0 || !ordered.some(d => d.state === 'queued' && d.deadlineMs > nowMs)} onClick={onApproveAll} type="button">Approve all {ordered.length}</button> : null}
      <div className="max-h-80 overflow-y-auto">
        {ordered.length === 0 ? <p className="p-4 text-xs text-slate-500">{actionable ? 'No flows are currently held.' : 'No current decisions loaded. See the control status below.'}</p> : null}
        {ordered.map((decision) => {
          const busy = inFlight.has(`decision:${decision.id}`)
          const expired = decision.state !== 'queued' || decision.deadlineMs <= nowMs
          ...
          return (
            <article className="border-b border-slate-800 p-3" key={decision.id}>
              <button className="block w-full text-left" onClick={() => onSelect(null, decision.flowKey)} type="button">
                ...
                <span className={`mt-1 block text-[11px] ${warning}`}>{hostname?.hostname || 'Hostname unknown'} · confidence {hostname?.confidence ?? 'none'} · {decision.packetCount} pkt · {decision.wireBytes ?? '—'} B wire / {decision.payloadBytes ?? '—'} B payload · flags 0x{decision.tcpFlags.toString(16)} · waiting {Math.floor(elapsed / 1000)}s · {expired ? 'Expired · awaiting reconciliation' : `watchdog in ${Math.ceil(Math.max(0, decision.deadlineMs - nowMs) / 1000)}s`}</span>
              </button>
              <div className="mt-2 flex gap-2">
                <button className="border border-emerald-700 px-2 py-1 text-xs text-emerald-300 disabled:opacity-40" disabled={!actionable || inFlight.size > 0 || expired} onClick={() => onApprove(decision.id)} type="button">{busy ? 'Working…' : 'Approve'}</button>
                <button className="border border-rose-700 px-2 py-1 text-xs text-rose-300 disabled:opacity-40" disabled={!actionable || inFlight.size > 0 || expired} onClick={() => onReject(decision.id)} type="button">Reject</button>
              </div>
            </article>
          )
        })}
