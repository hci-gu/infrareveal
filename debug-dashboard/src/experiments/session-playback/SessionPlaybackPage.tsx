...
function TrafficSession({ requestedId, preferenceKey }: { requestedId?: string; preferenceKey: string }) {
  ...
  const combined = useMemo<GatewayData>(() => ({ ...data, dnsQueries: merge(data.dnsQueries, activity.dnsQueries, evidence.dnsQueries), flowActivityChunks: merge(activity.chunks, evidence.chunks), flowActivityWindows: merge(activity.windows, evidence.windows), gateEvents: merge(activity.gateEvents, evidence.gateEvents) }), [activity.chunks, activity.dnsQueries, activity.gateEvents, activity.windows, data, evidence.chunks, evidence.dnsQueries, evidence.gateEvents, evidence.windows])
  ...
  return <main ref={rootRef} tabIndex={-1} className={`desktop-ui traffic-workspace ${expanded ? 'expanded' : ''} ${inspectorOpen ? '' : 'inspector-closed'} ${dockOpen ? '' : 'ledger-closed'}`} style={{ '--inspector-width': `${Math.max(240, Math.min(440, inspectorSize))}px`, '--ledger-height': `${Math.max(110, Math.min(350, dockSize))}px` } as CSSProperties} onKeyDown={event => {
    ...
    </div>{inspectorOpen ? <><Splitter label="Resize Traffic inspector" axis="vertical" value={inspectorSize} min={240} max={440} onChange={setInspectorSize} /><TrafficInspector shortfall={selection?.flowId ? shortfalls.get(selection.flowId) : undefined} data={combined} selected={selection} records={records} range={range} loading={rangePending || activity.loading || evidence.loading} error={activity.error || evidence.error} filtered={selectionFiltered} /></> : null}</div>
