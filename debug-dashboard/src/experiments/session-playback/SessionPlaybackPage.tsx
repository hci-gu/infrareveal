...
export function SessionPlaybackPage() {
  ...
  const [initialDeepLink] = useState(() => readTimelineLocation(sessionID))
  ...
  const selectedSessionId = sessionID && sessionID !== 'active' ? sessionID : initialDeepLink.sessionId
  ...
  function changeSession(value: string) {
    ...
    deepLinkAppliedRef.current = true
    navigate(`/timeline/${nextSessionId ?? 'active'}`)
    setTimelineUI({
      collapsedServiceIds: [],
      selectedClipId: null,
...
function readTimelineLocation(routeSessionId?: string) {
  ...
  return {
    sessionId: routeSessionId && routeSessionId !== 'active' ? routeSessionId : params.get('session'),
...
function writeTimelineLocation(sessionId: string | null, cursorMs: number | null) {
  ...
  if (sessionId) url.pathname = `/timeline/${encodeURIComponent(sessionId)}`
