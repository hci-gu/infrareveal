...
import { trackOpacity, trackTraffic } from '../map/mapTracks'
...
import { allowsDirection, directionColor, directionalBytes, directionLabel } from '../map/mapWorkspace'
...
import type { DestinationColumn, DestinationVolume, DestinationVolumeIndex, DirectionalColumn } from '../map/destinationVolumes'
...
export function MapComposition({ preferences, theme, workspace, overview, onWorkspace, onSeekTime, endMs, scene, trackCatalog, fps, mapStyleUrl, unavailable, loading, trafficIndex, trafficLoading, destinationIndex, destinationLoading, destinationError }: MapCompositionProps) {
  ...
  const trackFrame = overview
  const trackScene = overview.scene
  const equalControls = useRef<EqualEarthControls>(null)
  const equal = preferences.projection === 'equal-earth'
  const direction = workspace.direction
  ...
  const earthLocations = useMemo(() => activeOnly ? overview.locations.filter(location => location.connections.some(connection => connection.active)) : overview.locations, [activeOnly, overview.locations])
  ...
  const destinations = useMemo(() => destinationLoading ? [] : projectDestinationVolumes(trackScene, trackFrame.byFlow, destinationIndex, projectedCursorMs, visibleIPs), [destinationLoading, trackScene, trackFrame.byFlow, destinationIndex, projectedCursorMs, visibleIPs])
  ...
  const columns = useMemo(() => directionalColumns(destinations, direction), [destinations, direction])
  ...
  const destinationLabels = useMemo(() => {
    ...
    return destinations.filter(destination => {
      ...
      if (x < 35 || x > width - 55 || y < 90 || y > height - 90 || (x < 30)
  ...
  const trafficArcs = useMemo(() => directionalVolumeArcs(routes, trafficProfiles).filter(arc => allowsDirection(direction, arc.direction)), [routes, trafficProfiles, direction])
  const countryTraffic = useMemo(() => directionalVolumeArcs(countryRoutes, trafficProfiles).filter(arc => allowsDirection(direction, arc.direction)), [countryRoutes, trafficProfiles, direction])
  const tracedTraffic = useMemo(() => trafficPathEdges(directionalVolumeArcs(tracedPaths, trafficProfiles).filter(arc => allowsDirection(direction, arc.direction))), [tracedPaths, trafficProfiles, direction])
  ...
  const inspectorProfiles = useMemo(() => direction === 'both' ? trafficProfiles : new Map([...trafficProfiles].map(([id, profile]) => [id, { ...profile, rates: (direction === 'sent' ? profile.outRates : profile.inRates) ?? profile.rates.map(() => 0) }])), [trafficProfiles, direction])
  const selectedTraffic = selected ? trackTraffic(selected, trackScene, inspectorProfiles) : null
  ...
  const layers = useMemo(() => {
    if (equal || workspace.expanded) return []
    const routeOpacity = (arc: BundledMapArc) => trackOpacity(arc.trackId, selection)
    const neutralColor = (): Color => theme === 'light' ? [83, 111, 117] : [143, 171, 180]
    const flowColor = (value?: number): Color => directionColor(value ?? 1, theme === 'light')
    const arcProps = {
      ...
      updateTriggers: { getSourceColor: [selection, theme], getTargetColor: [selection, theme] },
    }
    const countryColor = (code: string): Color => {
      const destination = countryVolumes.get(code)
      return selection && destination?.tracks.some(track => track.trackId === selection) ? neutralColor() : TEAL
    }
    ...
    return [
      new CountryFootprintLayer({
        ...
        updateTriggers: { getFillColor: [selection, countryVolumes, theme] }, parameters: { depthCompare: 'always', depthWriteEnabled: false },
      }),
      new PathLayer({
        ...
        updateTriggers: { getColor: [selection, countryVolumes, theme] }, parameters: { depthCompare: 'always', depthWriteEnabled: false },
      }),
      new CountryArcLayer<BundledMapArc>({
        ...arcProps, id: 'country-connections', data: countryRoutes, getWidth: 1.5,
        getSourceColor: arc => alpha(neutralColor(), 0.5 * routeOpacity(arc)),
      ...
      new CountryFlowArcLayer<TrafficArc>({
        ...
        getSourceColor: arc => alpha(flowColor(arc.direction), routeOpacity(arc)),
        getTargetColor: arc => alpha(flowColor(arc.direction), routeOpacity(arc)),
      }),
      ...
      new ArcLayer<BundledMapArc>({
        ...
        getSourceColor: (arc) => alpha(neutralColor(), 0.06 * routeOpacity(arc)),
        getTargetColor: (arc) => alpha(neutralColor(), 0.04 * routeOpacity(arc)), getWidth: 3,
      }),
      new ArcLayer<BundledMapArc>({
        ...arcProps, data: measuredSpans, id: 'connection-strips', getWidth: 1,
        getSourceColor: (arc) => alpha(neutralColor(), 0.44 * routeOpacity(arc)),
        getTargetColor: (arc) => alpha(neutralColor(), 0.34 * routeOpacity(arc)),
      }),
      new GapArcLayer<BundledMapArc>({
        ...arcProps, data: unknownSpans, id: 'unknown-route-spans', getWidth: 1,
        getSourceColor: (arc) => alpha(neutralColor(), 0.48 * routeOpacity(arc)),
        getTargetColor: (arc) => alpha(neutralColor(), 0.38 * routeOpacity(arc)),
      }),
      new PathLayer<TrafficPathStrip>({
        id: 'traceroute-strips', data: tracedStrips, getPath: strip => strip.path,
        getColor: strip => alpha(neutralColor(), 0.5 * trackOpacity(strip.trackId, selection)),
      ...
      ...volumeLayers.map(({ focused, data }) => new FlowArcLayer<TrafficArc>({
        ...
        getDirection: arc => arc.direction ?? 1,
        getSourceColor: (arc) => alpha(flowColor(arc.direction), routeOpacity(arc)),
        getTargetColor: (arc) => alpha(flowColor(arc.direction), 0.92 * routeOpacity(arc)),
      ...
      ...pathVolumeLayers.map(({ focused, data }) => new FlowPathLayer<TrafficPathEdge>({
        ...
        getSourceColor: edge => alpha(flowColor(edge.direction), trackOpacity(edge.trackId, selection)),
        getTargetColor: edge => alpha(flowColor(edge.direction), trackOpacity(edge.trackId, selection)),
        visible: showTraffic, updateTriggers: { getSourceColor: [selection, theme], getTargetColor: [selection, theme] },
        parameters: { depthCompare: 'less-equal', depthWriteEnabled: focused },
      })),
      new ScatterplotLayer<MapHopPoint>({
        ...
        getLineColor: (hop) => alpha(neutralColor(), 0.85 * trackOpacity(hop.trackId, selection)),
        getLineWidth: 1.5, lineWidthUnits: 'pixels',
        updateTriggers: { getLineColor: [selection, theme], getFillColor: [selection, theme] }, parameters: { depthCompare: 'always', depthWriteEnabled: false },
      }),
      new TextLayer<MapHopPoint>({
        id: 'route-hop-order', data: visibleHops.filter(hop => hop.trackId === selection),
        visible: showTraffic && preferences.labels && viewState.zoom >= 3, getPosition: hop => hop.position,
      ...
      new ScatterplotLayer<MapPoint>({
        ...
        getFillColor: (point) => alpha(neutralColor(), (point.activeFlowCount > 0 ? 0.08 : 0.04) * trackOpacity(point.trackId, selection)),
        updateTriggers: { getRadius: selection, getFillColor: [selection, theme] }, parameters: { depthCompare: 'always' },
      }),
      new ScatterplotLayer<MapPoint>({
        ...
        getFillColor: (point) => alpha(neutralColor(), (point.activeFlowCount > 0 ? 1 : 0.55) * trackOpacity(point.trackId, selection)),
        getLineColor: (point) => point.trackId === selection ? [230, 255, 248, 255] : [11, 24, 34, 230 * trackOpacity(point.trackId, selection)],
        getLineWidth: 1.5, lineWidthUnits: 'pixels',
        updateTriggers: { getRadius: selection, getFillColor: [selection, theme], getLineColor: [selection, theme] }, parameters: { depthCompare: 'always' },
      }),
      ...columnLayers.map(({ focused, data }) => new ColumnLayer<DirectionalColumn>({
        ...
        updateTriggers: { getPosition: viewState.zoom, getElevation: viewState.zoom, getFillColor: [selection, theme] },
        parameters: { depthCompare: 'less-equal', depthWriteEnabled: focused },
      })),
      new TextLayer<DestinationVolume>({
        ...
        getText: destination => `${destination.estimated ? '≈ ' : ''}${direction === 'sent' ? '↑ ' : direction === 'received' ? '↓ ' : ''}${formatBytes(directionalBytes(destination, direction))}`,
        visible: preferences.labels,
        characterSet: '0123456789. BkMGT≈↓↑',
      ...
      new TextLayer({
        id: 'gateway-label', visible: preferences.labels, data: origin, getPosition: (point) => point.position,
  ...
  }, [cursorMs, fps, frame, routes, measuredSpans, unknownSpans, tracedStrips, pathVolumeLayers, origin, reducedMotion, selection, showTraffic, trafficAnchorMs, visibleHops, viewState.zoom, volumeLayers, columnLayers, destinationLabels, countryPolygons, countryBorders, countryVolumes, countryRoutes, countryTraffic, cityPoints, direction, theme, equal, workspace.expanded, preferences.labels])
  ...
  function fitNetwork(trackId?: string) {
    ...
    if (equal) { equalControls.current?.fit(positions); return }
    ...
    const padding = compositionWidth > 760 ? { left: 45, right: selection && compositionWidth > 1000 ? 350 : 60, top: 100, bottom: 100 } : { left: 50, right: 50, top: 75, bottom: 220 }
  ...
  return (
    <AbsoluteFill className="atlas-composition" data-map-zoom={viewState.zoom.toFixed(2)} data-map-pitch={viewState.pitch} data-route-mode={showRoutes ? 'traceroute' : 'direct'} data-selected-track={selection || undefined} data-track-count={trackFrame.tracks.length} data-destination-count={destinations.length} data-country-count={countryDestinations.length} data-city-column-count={columns.length} data-destination-bytes={Math.round(destinations.reduce((sum, destination) => sum + directionalBytes(destination, direction), 0))} data-workspace={workspace.expanded ? 'timeline' : 'map'} data-location-filter={workspace.locationId ?? ''}>
      ...
      {equal ? <EqualEarthMap controlsRef={equalControls} locations={earthLocations} origin={scene.origin} direction={direction} selected={workspace.locationId} onSelect={selectLocation} labels={preferences.labels} showTraffic={showTraffic} /> : <><DeckGL
        ...
        layers={layers} getTooltip={info => {
          if (info.layer?.id === 'country-footprints' && info.object) {
            ...
            return destination ? { text: `${destination.location} · Country estimate\nCity unknown\n${formatBytes(directionalBytes(destination, direction))} ${directionLabel(direction).toLowerCase()}\nClick for country totals and tracks` } : null
          }
          return tooltipForPoint(info, direction)
        }}
        onClick={({ object, layer }) => {
          if (layer?.id === 'country-footprints' && object) { const id = countryVolumes.get(object.countryCode)?.id ?? null; setSelectedCountry(id); onWorkspace({ ...workspace, locationId: id }); return }
          if (!object) return
          if (layer?.id === 'route-hops') { selectTrack((object as MapHopPoint).trackId ?? null); return }
          if (['destinations', 'destination-columns', 'muted-destination-columns'].includes(layer?.id ?? '')) {
            const locationId = 'destination' in object ? (object as DestinationColumn).destination.id : (object as MapPoint).position.join(',')
            selectLocation(workspace.locationId === locationId ? null : locationId)
          } }}
      ...
      <div className="atlas-map-heading"><MapIcon name="globe" size={15} /><span>NETWORK ATLAS</span><i /><span className="atlas-map-heading-detail">{equal ? 'Equal Earth' : 'Mercator'} · {directionLabel(direction)}</span></div>
      <div className="atlas-view-options">
        {!equal && <div className="atlas-segmented atlas-view-switch" aria-label="Map perspective">
        ...
        {!equal && <button type="button" className="atlas-layer-button" aria-pressed={showRoutes} onClick={() => setShowRoutes(value => !value)} title={showRoutes ? 'Show direct connections' : 'Show traceroute paths'}><MapIcon name="route" size={16} /><span>Routes</span><i className="atlas-dot" /></button>}
      ...
      {!equal && mapError && <div className="atlas-map-error" role="status">Basemap tiles are unavailable. Traffic is still shown.</div>}

      {!equal && <div className="atlas-map-tools" aria-label="Map controls">
      ...
      <div className="atlas-map-footer"><div className="atlas-legend"><span className="atlas-down"><i className="atlas-direction-swatch" />↓ Downloaded · to your devices</span><span className="atlas-up"><i className="atlas-direction-swatch is-sent" />↑ Sent · from your devices</span>{countryDestinations.length > 0 && <span><i className="atlas-country-swatch" />Country estimate</span>}</div><p className="atlas-column-scale" data-volume-source={destinationLoading ? 'loading' : destinationError ? 'unavailable' : destinationEstimated ? 'estimated' : destinationPartial ? 'partial' : 'captured'}>{equal ? 'Paired bars · shared linear scale' : 'Paired columns · fixed log scale'} · {directionLabel(direction)}{destinationLoading ? ' · Loading…' : destinationError ? ' · Capture unavailable' : destinationEstimated ? ' · ≈ Includes estimates' : destinationPartial ? ' · Partial capture' : ''}</p><p>{equal ? 'Simplified connections · country anchors are approximate' : !showRoutes ? 'Simplified connections' : hasUnknownSpans ? 'Dashed spans = unknown path' : 'Approximate routes'} · Coarse IP locations{!equal && (trafficLoading || estimatedTraffic || partialTraffic) ? trafficLoading ? ' · Loading rates' : estimatedTraffic ? ' · Estimated rates' : ' · Partial rates' : ''}</p></div>
      </div>
...
function tooltipForPoint({ object, layer }: PickingInfo<MapPoint | MapHopPoint | DestinationColumn>, direction: WorkspaceState['direction']) {
  ...
  const lines = 'destination' in object && destination
    ? [destination.location, `${formatBytes(directionalBytes(destination, direction))} ${directionLabel(direction).toLowerCase()}`,
      direction !== 'sent' ? `↓ ${formatBytes(destination.received)} downloaded from this location` : '', direction !== 'received' ? `↑ ${formatBytes(destination.sent)} sent to this location` : '',
    ...
    : 'ip' in object ? [object.label, object.location, object.ip, `${object.flowCount.toLocaleString()} flows`, 'Click to filter this location'] : []
  return {
