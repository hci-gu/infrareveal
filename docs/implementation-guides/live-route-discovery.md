...
Defaults: 20 automatic targets/session; one primary plus one ICMP Paris alternate at most; 40 automatic attempts/session and 40 total admissions/network/family/rolling hour; ten manual attempts/session. One physical worker lease includes cancellation and pipe drain. Scamper is pinned to Debian Bookworm `20211212-1.1`; one logical task uses five probes/second, two queries per TTL and a 45-second deadline. The task fits its depth to the serial reply waits: 20 TTLs/40 probes at the default deadline, with five seconds reserved for startup and completion (absolute ceiling 32 TTLs/64 probes). The remaining tail is explicitly unprobed. Engine control traffic is distinct from measurement probes.

Activity qualification admits the bounded comparison. Once its first method starts, the remaining method does not require a fresh traffic burst and takes priority over new automatic candidates. Existing session/hour/network limits still apply; no third method is scheduled.
...
A useful path has a public intermediate interface beyond established access context. A continuous initial segment followed by a silent span and possibly the endpoint is insufficient: it produces an access-only outcome and permits the bounded alternate, even before access consensus is established. A public intermediate responder beyond that initial segment can establish a useful sparse path. Geography is irrelevant to acceptance. An access prefix requires matching leading TTL/address positions from three distinct destination IPs, with supporting measurement IDs and timestamps. Historical records remain intact; old cache entries are rechecked before reuse.
